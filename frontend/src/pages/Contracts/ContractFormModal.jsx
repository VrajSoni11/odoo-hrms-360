...
export default function ContractFormModal({ contract, defaultEmployeeId, onClose, onSaved }) {
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit}>
          <label>Employee
            <select required value={form.employeeId} onChange={handleChange('employeeId')} disabled={!!defaultEmployeeId}>
          ...
          <div className="form-row">
            <label>Start Date<input type="date" required value={form.startDate} onChange={handleChange('startDate')} /></label>
            <label>End Date (leave blank = open-ended)<input type="date" value={form.endDate} onChange={handleChange('endDate')} /></label>
          </div>

          <label>Wage (monthly)<input type="number" min="0" step="0.01" required value={form.wage} onChange={handleChange('wage')} /></label>
          ...
          {error && (
            <div className="form-error">
              {error}
              <div className="form-error-hint">Only one ACTIVE contract per employee per date range is allowed — this is enforced at the database level.</div>
            </div>
          )}
